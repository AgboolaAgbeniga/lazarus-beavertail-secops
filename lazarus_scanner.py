...
IOC_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"invisibleferret", re.IGNORECASE),      "InvisibleFerret backdoor reference"),
    (re.compile(r"tasks\.json.*(curl|wget|powershell|python|node|sh|bash)", re.IGNORECASE), "Suspicious auto-run task in VS Code"),
    (re.compile(r"(curl|wget|powershell|python|node|sh|bash).*(\.woff|\.woff2)", re.IGNORECASE), "Execution of font file payload"),
    (re.compile(r"runOptions.*runOn.*folderOpen", re.IGNORECASE), "VS Code automatic task execution on folder open"),
]
...
PROCESS_CMDLINE_IOCS: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"invisibleferret", re.IGNORECASE),  "InvisibleFerret backdoor in cmdline"),
    (re.compile(r"tasks\.json", re.IGNORECASE),      "VS Code automated task execution"),
    (re.compile(r"\.woff|\.woff2", re.IGNORECASE),   "Fake font payload execution"),
]
...
def iter_targets(root: Path) -> Iterator[Path]:
    for dirpath, dirnames, filenames in os.walk(root, followlinks=False):
        ...
        for fn in filenames:
            if fn in CONFIG_FILE_NAMES or fn.endswith(".woff") or fn.endswith(".woff2"):
